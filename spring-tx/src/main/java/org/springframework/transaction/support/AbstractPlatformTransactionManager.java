...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		//判断当前线程是否存在事务，判断依据为当前线程记录的链接不为空且连接中（connectionHolder）中的transactionActive属性不为空
		...
		//如果当前线程不存在事务， 但是propagationBehavior 却被声明为(PROPAGATION MANDATORY()-使用当前的事务，如果当前没有事务，就抛出异常) 抛出异常
		...
		//PROPAGATION_REQUIRED 、PROPAGATION_REQUIRES_NEW 、PROPAGATION_NESTED 都需要新建事务
			...
			try {
				...
				/*
					＊ 构造transaction ，包括设置connectionHolder、隔离级别、timout，这里不是spring设置的而是委托给底层的数据库连接
					＊ 如果是新连接,绑定到当前线程
				*/
	...
	private TransactionStatus handleExistingTransaction(
		...
		//如果当前不支持事务，那就挂起已经存在的事务，以非事务方式运行
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				//如果可以使用保存点的方式控制事务回滚，那么在嵌套式的事务建立初期建立保存点
	 ...
	 * @see #resume
	 */
	//挂起当前事务，做法是保存当前事务的老属性比如name，readOnly，isolationLevel，wasActive然后将新属性设置为null或false
	@Nullable
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		//这种情况是有嵌套事务的情况下，假设要提交外层事务的时候，发现内层事务标记了回滚，那就直接回滚
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				//在提交之前添加的TransactionSynchronization中的对应方法的调用
				triggerBeforeCommit(status);
				//在完成提交之前添加的TransactionSynchronization中的对应方法的调用
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			try {
				//在事务回滚前激活所有TransactionSynchronization中的方法
				triggerBeforeCompletion(status);
			...
			//在事务回滚后激活所有TransactionSynchronization中的方法
			triggerAfterCompletion(status, TransactionSynchronization.STATUS_ROLLED_BACK);
